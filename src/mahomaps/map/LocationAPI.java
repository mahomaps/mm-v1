		...
		public void locationUpdated(LocationProvider provider, Location location) {
			...
			satellites:
			{
				...
				try {
					...
					if (s1 != null && s2 != null) {
						geoUpdateThread.totalSattelitesInView = Integer.parseInt(s1);
						geoUpdateThread.sattelites = Integer.parseInt(s2);
						break satellites;
					}
				} catch (Exception e) {
				}
				...
				if (nmea != null) {
					...
					for (int i = sequence.length - 1; i >= 0; i--) {
						...
						String[] sentence = GeoUpdateThread.split(s, ',');
			...
			if (location.isValid()) {
				...
				if (coordinates.getLatitude() != 0 && coordinates.getLongitude() != 0) {
					...
					geoUpdateThread.lastUpdateTime = location.getTimestamp();
